...
function StatusPill({ value }: { value?: string | null }) {
  const cfg = STATUS_LABELS[value || "not_started"] ?? STATUS_LABELS.not_started;
  return (
    <span
...
function TimelinePill({ value }: { value?: string | null }) {
  const cfg = TIMELINE_LABELS[value || "on_time"] ?? TIMELINE_LABELS.on_time;
  return (
    <span
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#1a2744]">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#1a4d2e] opacity-40" />
                <span className="relative inline-flex rounded-full h-2 w-2 bg-[#1a4d2e]" />
              </span>
                    ...
                    {projects.map((project, idx) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-center">
                          <StatusPill
                            value={project.feasibility_status}
                          />
                        </td>
                        <td className="px-3 py-2 text-center">
                          <StatusPill
                            value={project.financial_model_status}
                          />
                        </td>
                        <td className="px-3 py-2 text-center">
                          <StatusPill value={project.epc_status} />
